...
const systemDesignVisuals = {
  'CAP theorem':systemDesignVisual(['Client','write v2'],['Replica A','reachable'],['Partition','link down'],['Replica B','isolated'],'A client writes while both replicas initially hold v1.','The write reaches Replica A as communication to B fails.','The partition forces A to reject for consistency or accept for availability.','After healing, the system resumes one history or reconciles divergent versions.'),
  'PACELC':systemDesignVisual(['Request','operation'],['Partition path','P: A or C'],['Healthy path','E: L or C'],['Policy','chosen guarantee'],'Classify whether replicas can currently communicate.','Under partition, choose availability or coordinated consistency.','Without partition, choose lower latency or stronger replica agreement.','Apply the selected policy per operation and document both choices.'),
  'Consistency models':systemDesignVisual(['Write','v2'],['Replica set','visibility'],['Reader','observation'],['Contract','allowed history'],'A write creates a new version under concurrent access.','The replication protocol controls when each copy exposes v2.','A reader may observe v1, v2, or an ordered sequence.','The model declares exactly which observed histories are legal.'),
  'Strong consistency':systemDesignVisual(['Writer','put v2'],['Authority','serialize'],['Replicas','acknowledge'],['Reader','gets v2'],'The writer submits v2 to the authoritative path.','The authority orders v2 after prior operations.','Required replicas acknowledge before success is returned.','A subsequent read is routed or delayed until it can return v2.'),
  'Linearizability':systemDesignVisual(['Call A','write begins'],['Linearization point','v1 -> v2'],['Call A','write returns'],['Call B','read v2'],'A write is invoked while the object contains v1.','At one instant between invocation and response, the object becomes v2.','The completed write establishes a real-time boundary.','Any later read must observe v2 or a newer value.'),
  ...
  'Causal consistency':systemDesignVisual(['Post','event A'],['Reply','event B depends on A'],['Concurrent post','event C'],['Readers','causal view'],'A user publishes event A.','A reply B carries a dependency on A.','Independent event C occurs concurrently without that dependency.','Readers must see A before B, but may place C on either side.'),
  'Eventual consistency':systemDesignVisual(['Primary','v1 -> v2'],['Replica A','v1 at T0'],['Replica B','v1 at T0'],['Convergence','all v2 at Tn'],'The primary accepts v2 while replicas still expose v1.','Replica A receives and applies v2 after propagation delay.','Replica B remains stale briefly, then receives the update.','With no new writes, every replica converges to v2 at Tn.'),
  'Session consistency':systemDesignVisual(['Session','version token 7'],['Router','minimum version'],['Replica A','version 6'],['Replica B','version 7'],'A session records the version returned by a successful operation.','The next request carries version token 7 to the router.','Replica A is rejected or awaited because it only has version 6.','Replica B serves the request while satisfying the session guarantee.'),
  ...
  'Partition tolerance':systemDesignVisual(['Node A','healthy'],['Network link','partitioned'],['Node B','healthy'],['Policy','degrade safely'],'Both nodes operate normally while exchanging state.','The link fails although both processes remain healthy.','Each side follows quorum or ownership rules instead of guessing peer death.','The service preserves its declared safety or availability behavior until healing.'),
  'Quorum reads/writes':systemDesignVisual(['Coordinator','N=3'],['Write quorum','W=2'],['Read quorum','R=2'],['Resolver','latest version'],'A coordinator maps the key to three replicas.','A write succeeds after two replicas store the version.','A later read collects two replica responses.','Because R+W>N, the resolver can encounter and select the written version.'),
  'Read repair':systemDesignVisual(['Reader','get key'],['Replica A','v3'],['Replica B','v2'],['Repair','write v3'],'A read queries multiple replicas for the same key.','Replica A returns the newer version v3.','Replica B reveals that it still stores v2.','The coordinator returns v3 and repairs Replica B in the background.'),
  'Anti-entropy':systemDesignVisual(['Replica A','Merkle root X'],['Replica B','Merkle root Y'],['Range diff','keys 40-63'],['Repair stream','missing versions'],'Replicas periodically compare compact state summaries.','Different roots prove that their contents diverge.','They descend matching trees to isolate the differing key range.','Only missing or stale versions are exchanged until summaries agree.'),
  ...
  'Failure detectors':systemDesignVisual(['Peer','heartbeat'],['Detector','last seen T0'],['Timeout','suspicion rises'],['Membership','suspect not proof'],'A peer periodically emits heartbeats.','The detector records the last successful evidence of liveness.','Missing heartbeats increase suspicion after a configured delay.','Membership treats failure as a suspicion and uses quorum or epochs before acting.'),
  'FLP impossibility':systemDesignVisual(['Proposer A','value X'],['Proposer B','value Y'],['Network','unbounded delay'],['Decision','cannot be guaranteed'],'Two correct participants propose different values.','Messages are delayed so neither can distinguish delay from a crash.','An adversarial schedule can keep the protocol in an undecided configuration.','Practical consensus adds eventual timing assumptions or randomness for liveness.'),
  'Split brain':systemDesignVisual(['Leader A','accepts v2'],['Partition','no contact'],['Leader B','accepts v3'],['Recovery','conflict'],'A healthy cluster starts with one authority.','A partition causes both sides to believe the other failed.','Without fencing, A and B each accept conflicting writes.','Recovery must reject a stale leader or reconcile two divergent histories.'),
  'Byzantine vs crash failures':systemDesignVisual(['Crash node','silent'],['Byzantine node','conflicting replies'],['Quorum','authenticated votes'],['Decision','fault model met'],'A crash-failed node simply stops responding.','A Byzantine node can send different false values to different peers.','The protocol gathers authenticated votes sized for the assumed fault type.','A decision is safe only when replica count and quorum match that model.'),
  ...
  'Idempotency':systemDesignVisual(['Client','operation key K'],['Service','dedupe lookup'],['State store','result R'],['Retry','returns R'],'A client submits an operation with stable key K.','The service atomically checks whether K already has an outcome.','The first execution commits the effect and stores result R under K.','A retry finds K and returns R without repeating the effect.'),
  ...
  'FIFO ordering':systemDesignVisual(['Producer P','M1 then M2'],['Partition','append 10,11'],['Consumer','offset 10'],['Consumer','offset 11'],'Producer P sends M1 before M2.','Both messages enter the same ordered partition at consecutive offsets.','The consumer processes and commits M1 first.','Only then does it advance to M2, preserving producer order.'),
  'Causal ordering':systemDesignVisual(['Event A','created'],['Event B','depends on A'],['Dependency metadata','A included'],['Delivery','A before B'],'Event A occurs and receives a causal identifier.','A handler creates B while observing A.','B carries dependency metadata proving the relationship.','A receiver delays B until A is present, while concurrent events remain flexible.'),
  ...
  'Asynchronous replication':systemDesignVisual(['Leader','commit v9'],['Client','early success'],['Replica','still v8'],['Shipping','replica reaches v9'],'The leader commits v9 locally.','The client receives success without waiting for a remote copy.','A follower may temporarily serve stale v8.','Background log shipping eventually applies v9 unless the leader fails first.'),
  ...
  'CRDTs':systemDesignVisual(['Replica A','add x'],['Replica B','add y'],['Merge','join states'],['Convergence','{x,y}'],'Replica A updates its local CRDT without coordination.','Replica B concurrently performs another valid update.','Replicas exchange states or commutative operations.','Associative, commutative, idempotent merge makes both converge on the same value.'),
  'Active-active replication':systemDesignVisual(['Region A','serves traffic'],['Region B','serves traffic'],['Replication','bidirectional'],['Conflict policy','converged state'],'Both regions accept local reads and writes.','Each region commits against its local authority or partition.','Changes replicate bidirectionally across the long-distance link.','Conflicts are prevented by key ownership or resolved to one converged state.'),
  'Active-passive replication':systemDesignVisual(['Active region','serves writes'],['Passive region','replays log'],['Outage','active unavailable'],['Promotion','passive becomes active'],'The active region handles traffic and emits a replication stream.','The passive region continuously replays state and measures lag.','A declared outage triggers fencing and recovery checks.','Traffic moves after the passive is promoted under a new epoch.'),
  'Cross-region replication':systemDesignVisual(['Region A','authoritative write'],['WAN','high latency link'],['Region B','replica lag'],['Failover','RPO evaluated'],'Region A commits an authoritative update.','The update crosses a variable-latency regional link.','Region B applies it and reports a recovery position.','A failover decision compares that position with the promised RPO.'),
  ...
  'Directory-based partitioning':systemDesignVisual(['Key K','lookup'],['Directory','K -> shard C, epoch 9'],['Shard C','serve'],['Migration','epoch 10 redirect'],'The router asks a directory for key K placement.','The directory returns Shard C with mapping epoch 9.','Shard C validates the epoch and serves the operation.','A migration publishes epoch 10 and stale routers follow a redirect.'),
  'Virtual nodes':systemDesignVisual(['Physical node A','tokens 10,90'],['Physical node B','tokens 40,130'],['Keys','many intervals'],['Rebalance','selected tokens move'],'Each physical node owns several virtual tokens.','Keys distribute across token intervals rather than one contiguous node range.','Capacity weights assign more tokens to larger nodes.','Rebalancing moves selected token ranges across many peers instead of one huge range.'),
  ...
  'Hot partitions':systemDesignVisual(['Metrics','shard 7 saturated'],['Router','key distribution'],['Shard split','7a/7b'],['Traffic','balanced'],'Per-shard metrics identify shard 7 as the bottleneck.','Key-level analysis distinguishes traffic skew from total data size.','The system splits, replicates, or isolates the hot range.','A versioned route shifts load while preserving ownership correctness.'),
  'Hot keys':systemDesignVisual(['Key K','40% traffic'],['Owner shard','CPU saturated'],['Mitigation','replicas/coalescing'],['Clients','distributed load'],'Metrics identify one key rather than the whole shard as hot.','Its single owner saturates under repeated reads or writes.','Read copies, request coalescing, salting, or aggregation handle the exact access pattern.','Routing distributes safe work while retaining required ordering.'),
  ...
  'Partition affinity':systemDesignVisual(['Aggregate key','order-17'],['Router','stable owner'],['Shard C','state + cache'],['Failover','new owner epoch'],'All operations extract the same aggregate key.','Stable routing sends them to one owner.','Local state, order, and cache reuse avoid cross-shard coordination.','Failover transfers the affinity under a new epoch before processing resumes.'),
  'Tenant-based partitioning':systemDesignVisual(['Tenant ID','tenant-8'],['Tenant map','cell B/shard 4'],['Quota','tenant scoped'],['Migration','tenant remapped'],'Every request is authenticated to a tenant identity.','A directory maps that tenant to an isolation cell and shard.','Storage, quota, and noisy-neighbor controls apply within that boundary.','Migration copies and cuts over the tenant as one governed unit.'),
  ...
  'Write-through cache':systemDesignVisual(['Caller','put v4'],['Cache','update v4'],['Database','persist v4'],['Ack','both complete'],'A caller submits v4 through the cache interface.','The cache records or reserves the new version.','The backing store synchronously persists v4.','Success is returned only after the write-through policy completes both paths.'),
  ...
  'TTL':systemDesignVisual(['Entry','stored at T0'],['TTL','expires T0+60s'],['Reader','checks freshness'],['Miss','reload + jitter'],'An entry is stored with an absolute or relative expiry.','Time advances while reads can use the still-valid value.','A read after expiry treats the entry as absent.','The loader refreshes it with a jittered next TTL to avoid synchronized expiry.'),
  ...
  'Event streaming':systemDesignVisual(['Producer','append event'],['Partition log','ordered record'],['Processor','state update'],['Checkpoint','offset + state'],'A producer appends an immutable event to a keyed partition.','The log assigns an ordered position and retains the event.','A stream processor updates keyed state in position order.','A checkpoint binds source progress to recoverable processor state.'),
  ...
  'Offsets':systemDesignVisual(['Partition','records 20..23'],['Offset 21','stable position'],['Fetch','start 21'],['Advance','next 22'],'The broker appends records at monotonically increasing partition positions.','Offset 21 uniquely names one record within that partition.','A consumer requests data beginning at 21.','After handling it, progress advances to the next required offset.'),
  'Consumer offsets':systemDesignVisual(['Consumer','process offset 50'],['Business store','effect commits'],['Offset store','commit 51'],['Recovery','resume 51'],'The consumer receives the record at offset 50.','It commits the intended business effect.','It records 51 as the next position only after the effect is durable.','After restart, consumption resumes at 51; a crash between commits may safely duplicate 50.'),
  'Replay':systemDesignVisual(['Retained log','offset 0..N'],['Consumer','reset cursor'],['Projection','rebuild state'],['Cutover','caught up'],'A retained immutable log remains available after original processing.','The consumer resets to a chosen historical offset.','Replay applies version-aware, idempotent logic into a new projection.','After catching the live tail, the rebuilt projection becomes active.'),
  'Retention':systemDesignVisual(['Log segments','dated records'],['Policy','7 days/size cap'],['Cleaner','eligible segments'],['Storage','old data removed'],'The broker rolls appended records into immutable segments.','Time and size policy marks old segments as no longer replayable.','The cleaner waits for required safety or compliance conditions.','Eligible segments are deleted and storage usage falls.'),
  'Compaction':systemDesignVisual(['Log','K:v1,K:v2,K:v3'],['Cleaner','group by key'],['Latest record','K:v3'],['Compacted log','history reduced'],'The append log accumulates several versions for key K.','A background cleaner identifies superseded records by key.','The latest value or retained tombstone remains authoritative.','A compacted segment preserves recoverable current state while dropping intermediate versions.'),
  ...
  'Message ordering':systemDesignVisual(['Producer','events A,B'],['Partition','offsets 8,9'],['Consumer','expected 8'],['State','apply A then B'],'Related events share the same ordering key.','The broker appends A at 8 and B at 9.','The consumer tracks the next expected offset and handles gaps or retries.','State changes apply A before B even across consumer recovery.'),
  'Exactly-once processing':systemDesignVisual(['Input offset','37'],['Processor state','update S'],['Output log','event O'],['Transaction','commit all'],'The processor reads input offset 37.','It computes state update S and output O.','State, output, and next input offset join one atomic broker or state-store transaction.','Recovery sees either all committed or none and retries without a second logical result.'),
  'Idempotent consumers':systemDesignVisual(['Message','ID M7'],['Inbox','lookup M7'],['Effect','commit once'],['Duplicate','returns prior outcome'],'The consumer receives stable identity M7.','It checks an inbox record inside the effect transaction.','The first delivery writes the business effect and M7 marker atomically.','A duplicate finds the marker and skips the effect while acknowledging safely.'),
  'Transactional messaging':systemDesignVisual(['Transaction','begin'],['Input progress','stage'],['Outputs','stage messages'],['Commit','atomic visibility'],'The processor begins a transaction supported by the broker boundary.','Consumed progress is staged but not yet visible.','Produced messages and state changes join the same transaction.','Commit exposes outputs and progress together; abort exposes neither.'),
  ...
  'Inbox pattern':systemDesignVisual(['Broker','deliver M'],['Inbox table','insert M ID'],['Business state','effect'],['Commit','dedupe + effect'],'The consumer receives message identity M.','It attempts a unique inbox insert within a local transaction.','If new, it applies the business effect in that transaction.','Commit makes marker and effect durable together; duplicates violate the insert and skip work.'),
  ...
  'Three-phase commit':systemDesignVisual(['Coordinator','canCommit?'],['Participants','preCommit'],['Timeout assumptions','bounded network'],['Commit','final decision'],'The coordinator gathers willingness to commit.','A pre-commit phase tells participants that unanimous readiness was reached.','Under bounded-delay and failure assumptions, participants use timeout state to progress.','The final commit completes, but partitions can invalidate those non-blocking assumptions.'),
  'XA transactions':systemDesignVisual(['Transaction manager','XID'],['Resource managers','XA prepare'],['Decision log','commit/rollback'],['Recovery','resolve XID'],'A transaction manager assigns a global XID across resources.','Each XA resource durably prepares its local branch.','The manager records one commit or rollback decision.','On restart, in-doubt branches query or replay that XID decision.'),
  'Saga pattern':systemDesignVisual(['Step A','local commit'],['Step B','local commit'],['Step C','fails'],['Compensation','undo B then A'],'The saga durably records and executes local step A.','It advances to committed step B without a global transaction.','Step C fails after earlier effects are visible.','The saga resumes compensating actions in reverse semantic order until resolved.'),
  ...
  'Orchestration':systemDesignVisual(['Orchestrator','workflow state'],['Service A','command/reply'],['Service B','command/reply'],['Recovery','resume next step'],'The orchestrator durably records the workflow and sends a command to A.','A executes idempotently and returns a correlated result.','The orchestrator advances state and commands B or chooses compensation.','After failure, it reloads state and resumes the exact pending transition.'),
  'Compensating transactions':systemDesignVisual(['Original step','reserve inventory'],['Later failure','payment rejected'],['Compensation','release inventory'],['Ledger','resolved'],'An earlier local transaction reserves inventory.','A later step fails after the reservation is visible.','An idempotent compensation releases the reservation under current business rules.','The workflow ledger records success or escalates an irreconcilable compensation.'),
  ...
  'MVCC':systemDesignVisual(['Row','versions v3,v4'],['Reader','snapshot at v3'],['Writer','creates v4'],['Vacuum','remove obsolete v3'],'A reader begins with a snapshot that sees version 3.','A concurrent writer creates and commits version 4 rather than overwriting v3 in place.','New readers see v4 while the original reader still sees v3.','Garbage collection removes v3 only after no active snapshot needs it.'),
  'Snapshot isolation':systemDesignVisual(['Transaction A','snapshot S'],['Transaction B','commits row Y'],['Transaction A','writes row X'],['Validation','write conflicts checked'],'A begins against a stable committed snapshot S.','B commits changes after S without altering what A reads.','A writes based on its snapshot while reading repeatably.','Commit rejects direct write conflicts, though disjoint writes can still create write skew.'),
  ...
  'Atomic commit':systemDesignVisual(['Coordinator','transaction T'],['Participants','durable votes'],['Decision','commit or abort'],['Recovery','same outcome everywhere'],'A multi-resource transaction receives one global identity.','Participants durably report whether they can commit.','The coordinator records one final outcome before announcing it.','Retries and recovery drive every participant to that same outcome.'),
  'Idempotent operations':systemDesignVisual(['Caller','operation ID 88'],['Service','conditional create'],['Ledger','outcome stored'],['Retry','same response'],'The caller assigns stable operation ID 88 before the first attempt.','The service conditionally creates the operation record.','Effect and outcome are committed against that identity.','Any retry reads the ledger and returns the existing result.'),
  'Timeouts':systemDesignVisual(['Caller','deadline T'],['Dependency','request running'],['Timer','expires'],['Caller','cancel/fail'],'The caller computes a per-attempt timeout within its end-to-end deadline.','The dependency begins work while the timer runs.','No response arrives before expiry, leaving the remote outcome potentially unknown.','The caller cancels if possible and applies a safe retry or failure policy.'),
  'Retries':systemDesignVisual(['Attempt 1','transient failure'],['Classifier','retryable + safe'],['Attempt 2','same idempotency key'],['Outcome','success or budget exhausted'],'The first remote attempt fails or times out.','A classifier confirms the failure is transient and repetition is safe.','A later attempt uses the same logical operation identity.','Processing stops on success, permanent failure, deadline, or retry-budget exhaustion.'),
  'Exponential backoff':systemDesignVisual(['Failure','attempt 1'],['Delay','base*2^n'],['Cap','maximum wait'],['Retry','after delay'],'A retryable failure increments the attempt number.','The next delay grows exponentially instead of retrying immediately.','A cap keeps delay and total deadline bounded.','The operation retries after waiting, reducing pressure on recovery.'),
  ...
  'Retry budgets':systemDesignVisual(['Normal traffic','100 requests'],['Budget','10 retry tokens'],['Failures','consume tokens'],['Limiter','stop retries'],'Normal traffic earns or defines a bounded retry allowance.','Each additional attempt consumes one retry token.','Transient failures spend the budget while capacity remains safe.','When exhausted, requests fail without adding more retry load.'),
  'Circuit breakers':systemDesignVisual(['Closed','calls flow'],['Open','failure threshold crossed'],['Half-open','limited probes'],['Closed/reopen','probe outcome'],'Calls flow normally while classified outcomes remain healthy.','A rolling failure threshold opens the circuit and calls fail fast.','After a cool-down, a few half-open probes test recovery.','Successful probes close the circuit; failures reopen it.'),
  'Hedged requests':systemDesignVisual(['Primary request','replica A'],['Hedge timer','p95 delay'],['Duplicate','replica B'],['Winner','first valid response'],'A read starts against replica A.','If it exceeds a chosen delay without finishing, the hedge timer fires.','The same safe request goes to independent replica B.','The first valid response wins and the slower attempt is cancelled or discarded.'),
  ...
  'Rate limiting':systemDesignVisual(['Request','tenant A'],['Limiter','tenant bucket'],['Decision','token available?'],['Service','admit or reject'],'A request arrives with an authenticated policy dimension.','The limiter loads the matching tenant or endpoint state.','Available budget is atomically consumed or the request is denied.','Only admitted work reaches the protected service, with retry guidance on rejection.'),
  'Adaptive throttling':systemDesignVisual(['Telemetry','latency/errors'],['Controller','target utilization'],['Limit','increase/decrease'],['Traffic','new admission rate'],'The controller observes latency, queueing, errors, and concurrency.','It compares those signals with a stable operating target.','The admission limit rises cautiously or falls quickly with hysteresis.','Subsequent traffic uses the new limit while the controller watches for oscillation.'),
  'Admission control':systemDesignVisual(['Request','cost + priority'],['Capacity','current permits'],['Gate','deadline feasible'],['Outcome','admit/defer/reject'],'A request declares or receives an estimated resource cost.','The gate checks current concurrency and downstream capacity.','Priority and remaining deadline determine whether timely completion is plausible.','The request is admitted, delayed within bounds, or rejected before expensive work.'),
  ...
  'Blast-radius reduction':systemDesignVisual(['Change','new version'],['Canary','1% scope'],['Cells','staged waves'],['Rollback','damage bounded'],'A potentially risky change starts in a tiny isolated scope.','Health and correctness signals are evaluated against abort thresholds.','Promotion proceeds through independent cells or tenant cohorts.','A bad signal stops and rolls back before reaching the remaining population.'),
  'Fail-fast':systemDesignVisual(['Request','arrives'],['Precondition','invalid/unavailable'],['Gate','immediate error'],['Capacity','preserved'],'A request reaches a boundary with explicit prerequisites.','Validation or dependency state proves useful completion is unlikely.','The service returns a clear error without queueing or retries.','Scarce capacity remains available for work that can succeed.'),
  'Fail-open':systemDesignVisual(['Request','policy check'],['Policy service','unavailable'],['Fallback rule','allow'],['Audit','degraded decision'],'A request requires a control-plane policy decision.','The policy service cannot respond before the deadline.','A predeclared fail-open rule permits the operation to preserve availability.','The degraded decision is bounded, audited, and reconciled after recovery.'),
  'Fail-closed':systemDesignVisual(['Request','authorization check'],['Authority','unavailable'],['Fallback rule','deny'],['Response','safe failure'],'A request requires proof of authorization or safety.','The authoritative dependency cannot provide that proof.','A fail-closed rule refuses to infer permission from absence.','The operation is denied until the dependency or an approved cached proof is available.'),
  'Graceful degradation':systemDesignVisual(['Request','full experience'],['Dependency','optional feature fails'],['Feature gate','disable enrichment'],['Response','core result'],'A request begins with core and optional work planned.','An optional dependency exceeds its error or latency threshold.','The service disables that bounded feature rather than retrying into overload.','It returns an accurate core response marked with degraded behavior.'),
