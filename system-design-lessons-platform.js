(function () {
  ...
  add(identityChapter, 'OBO', 'identity',
    ...
    [['user','Employee Browser','Delegating user'],['web','Dashboard API','Middle-tier confidential client'],['sts','Corporate STS','On-behalf-of exchanger'],['reports','Reports API','Downstream resource'],['audit','Delegation Audit','Actor-subject evidence']],
  ...
  add(traceChapter, 'OpenTelemetry', 'trace',
    ...
    [['app','Orders Process','OTel-instrumented workload'],['sdk','OTel SDK','Context and batching'],['agent','OTel Collector Agent','Node receiver and processor'],['gateway','OTel Gateway','Central sampling and export'],['backend','Telemetry Backend','Trace, metric, and log store']],
    [['app','sdk','OTel API calls'],['sdk','agent','OTLP/gRPC batches'],['agent','gateway','Filtered OTLP'],['gateway','backend','Vendor exporter']],
    [
      {t:'Describe the resource',a:['app','sdk','Set service resource attributes'],n:'The SDK records service.name=orders-api, service.version=42.3, deployment.environment=prod, cloud.region=westus2.',o:'All three signals share one workload identity.',s:{app:['running','orders-api 42.3; pod=orders-7f9'],sdk:['configured','resource schema=https://opentelemetry.io/schemas/1.27.0']}},
